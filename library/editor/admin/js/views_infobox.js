		...
		var atts = parent.tinymce.activeEditor.windowManager.getParams();
		  ...
		  $('#mce-upload').on('click', function( event ) {
		    ...
		    file_frame.on( 'select', function() {
		      ...
		      $("#mce-infobox-form .infobox img").remove();
		      ext = attachment.url.substr(attachment.url.length - 4);
		      thumb = attachment.url.replace(ext, window.imgSuffix + ext);
			  $("<img>").attr("src", thumb).data('imgid', attachment.id).prependTo( '#mce-infobox-form .infobox .img' );
		...
		// initial loading of the content from the parent window
	 	...
	 	$.ajax(idURL,{
				...
				$('#mce-infobox-form header .img').prepend( $imgEl );

          });

	 	
		$('#mce-infobox').val( atts.data.title );
		...
		$('#mce-update').on('click', function(e) {
			...
			data = {
				...
				//img: $('#mce-infobox-form header img').attr('src'),
				imgid: $('#mce-infobox-form header img').data('imgid'),
				title : $('#mce-infobox').val(),
